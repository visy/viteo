...
void __interrupt __far keyboardHandler()
 ...
 if(keyCode==0xE0){keyAdd=128;}
 else
  {
  if(keyCode>127)
   {
   keyCode&=127;keyCode+=keyAdd;singleHitOk[keyCode]=1;
   key[keyCode]=0;keyCode=0;
   }
  else
   {
   keyCode+=keyAdd;
   if(!key[keyCode])
    {
    if(singleHitKey[keyCode]){if(singleHitOk[keyCode]){key[keyCode]=1;}}
    else {key[keyCode]=1;}
    if(singleHitOk[keyCode])
     {
     singleHitOk[keyCode]=0;
     codeBuffer[codeBufferCtr]=keyCode;codeBufferCtr++;
     if(codeBufferCtr>=16)codeBufferCtr=0;
     }
    }
   }
  keyAdd=0;
...
char checkKey(short keyNum)
 ...
 if(keyNum==KEY_BREAK){return breakCheck();}else
 return key[keyNum];
 }
...
void singleHitProcess(void)
 {
 long i;
 for(i=0;i<256;i++){if(singleHitKey[i]){if(key[i]){key[i]=0;}}}
 }
